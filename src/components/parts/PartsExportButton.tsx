...
export default function PartsExportButton({ parts }: PartsExportButtonProps) {
  ...
  const handleDownloadCSV = () => {
    ...
    a.href = url;
    a.download = `life-parts-summary-${formatDate(new Date())}.csv`;
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10 bg-aviation-50">
                  <tr className="text-aviation-700 text-xs uppercase tracking-wider">
                    <th className="text-left px-4 py-3 font-semibold">风险</th>
                    <th className="text-left px-3 py-3 font-semibold">飞机号</th>
                    <th className="text-left px-3 py-3 font-semibold">件号 PN</th>
                    <th className="text-left px-3 py-3 font-semibold">序号 SN</th>
                    <th className="text-left px-3 py-3 font-semibold">名称</th>
                    <th className="text-left px-3 py-3 font-semibold">装机位置</th>
                    <th className="text-right px-3 py-3 font-semibold">剩余循环 FC</th>
                    <th className="text-right px-3 py-3 font-semibold">剩余天数</th>
                    <th className="text-left px-3 py-3 font-semibold">到寿日期</th>
                    <th className="text-left px-3 py-3 font-semibold">排程状态</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  {sortedParts.map((p, idx) => {
                    ...
                    return (
                      <tr key={p.id} className={`${rowBg} border-t border-gray-100`}>
                        ...
                        <td className="px-3 py-3 font-mono-tabular font-semibold text-aviation-700">
                          {p.aircraftReg}
                        </td>
                        <td className="px-3 py-3 font-mono-tabular text-gray-800">{p.partNumber}</td>
                        <td className="px-3 py-3 font-mono-tabular text-gray-600 text-xs">{p.serialNumber}</td>
                        ...
                        <td
                          className="px-3 py-3 text-right font-mono-tabular font-semibold"
                        ...
                        <td className="px-3 py-3 font-mono-tabular text-gray-700">{p.expiryDate}</td>
                        <td className="px-3 py-3 text-gray-600 text-xs">{SCHEDULE_LABEL[p.scheduleStatus]}</td>
                      </tr>
                    );
                  })}
                </tbody>
